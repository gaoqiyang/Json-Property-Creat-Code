...
+ (void)writeToFileWithFilePath:(NSString *)filePath{
    ...
    [myText writeToFile:@"/Users/mac/Desktop/MyModel/FMDB操作语句.m" atomically:YES encoding:NSUTF8StringEncoding error:nil];
}
+ (void)writeFileToModelWithFilePath:(NSString *)filePath{
    ...
    for (NSString *path in [FMDBCreat getAllFileNameWithPath:filePath]) {
        ...
        //这些是每个模型里面都自带的基本函数
        [myText appendString:@"删除:\n"];
        [myText appendString:[FMDBCreat modelCreatDeleteDatawithInsideValue:0 withClassName:[FMDBCreat getClassNameFromFilePath:path]]];
        [myText appendString:@"插入:\n"];
        [myText appendString:[FMDBCreat modelCreatInsertDataFromNameAndAtributesDic:dic withInsideValue:0 withClassName:[FMDBCreat getClassNameFromFilePath:path]]];
        [myText appendString:@"无条件查询:\n"];
        [myText appendString:[FMDBCreat modelCreatReadDataNoConditionsFromNameAndAtributesDic:dic withInsideValue:0 withClassName:[FMDBCreat getClassNameFromFilePath:path]]];
        [myText appendString:@"条件查询:\n"];
        [myText appendString:[FMDBCreat modelCreatReadDataHaveConditionsFromNameAndAtributesDic:dic withInsideValue:0 withClassName:[FMDBCreat getClassNameFromFilePath:path]]];
    }
    [myText writeToFile:@"/Users/mac/Desktop/MyModel/zhanghuan.m" atomically:YES encoding:NSUTF8StringEncoding error:nil];
}
